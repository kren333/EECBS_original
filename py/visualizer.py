...
def formatStrings(something) -> str:
    if isinstance(something, list) or isinstance(something, tuple) or \
                                        isinstance(something, np.ndarray):
        strs = []
        for x in something:
            if isinstance(something, np.ndarray) or isinstance(x, float):
                if abs(int(x) - x) < 1e-5:
                    strs.append("{}".format(int(x)))
                else:
                    strs.append("{:.2}".format(x))
            else:
                strs.append(str(x))
                # strs.append("{:.2}".format(x))
        return ",".join(strs)
    if isinstance(something, int) or isinstance(something,str):
        return something
    return "{:.2}".format(something)

def plotLH2(ax, obs, hvals, state, extraCost, deltaK, trapped, predVals=None):
    """Note: obs should have 0 free, 1 invalid"""
    D = hvals.shape[0]
    ...
    trueVals = [extraCost, deltaK, trapped]
    curTitle = "State: {}, T: {}".format(formatStrings(state), formatStrings(trueVals))
    if predVals is not None:
        curTitle += " F: {}".format(formatStrings(predVals))
    # if predVals is not None:
    #     ax.set_title("State: True: {}, Pred: {}".format(formatStrings(state),
    #                                              formatStrings(predVals)))
    # else:
    #     ax.set_title("True: {}".format(formatStrings(state)))
